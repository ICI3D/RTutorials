...
## Fun with one of my favorite distributions: The binomial distribution
...
##			sample space, discrete, random variable, probability distribution
## * know the characteristics of a binomial process
...
# that it provides us with two important pieces of information: the possible
...
# number of independent trials.  It's a good model for a process that: 
# 1) has a fixed number of independent trials
# 2) has outcomes that can be categorized into either "success" or "failure"
# 3) has a constant probability of "success" per trial
...
# (n=10).  There are only two outcomes (heads or tails). The probability of 
# a success (e.g. getting "tails") remains constant. The number of tails in 
# 10 trials is a random variable that follows a binomial distribution.

# What is a random variable?  Well, before we talk about that, let's define one
# other term - sample space.  The sample space of a random phenomenon is the set
# of all possible outcomes that could occur.  Note that an outcome does not need
# to be numeric.  For example, consider tossing a coin one time.  The sample space
# is {heads, tails}.  
# If you were to toss the coin two times, the sample space would consist of:
...
# Also, notice that the random variable X is binomial because we have: 
# 1) fixed number of independent trials (n=2), 
#	2) either a "success" (tails) or failure occurs (not tails) per trial, and
# 3) the probability of a "success" (getting tails) remains constant.
#    Here the probability of a success in any one trial is p=0.5
#		 if we are tossing a fair coin.	
...
# example, if we didn't know that the probability of getting 0 tails on 2 flips
...
# x = number of successes (the observed value of the random variable X)
# n = number of trials (an input value describing the process)
# p = the probability of a success per trial (an input value describing 
#			the process)

# In our example, n = 2 and p = 0.5.  To calculate the probability that X = 0
# (i.e. that we will not obtain any tails) use:
# P(X = 0) = 2!/[(0!)(2-0)!] * 0.5^0 * (1-0.5)^(2-0)
#          = 1               * 1     * 0.25         = 0.25    (recall that 0! = 1)

# This has all been a preamble towards getting you prepared to use the binomial
...
# See the help file using help(dbinom)
...
# times.  Instead of calculating it with the formula (or a calculator), just 
# use R.  Here, n=50, p is still 0.5. Run:
...
# We will use base R plotting commands in this tutorial, and in Tutorial 4,
# that we will do together during MMED, we will practice using the ggplot2
...
# parameters, the mean and the standard deviation.  To see how the distribution
# changes as p changes, try highlighting and running the following code (here
# we're holding n constant at 50), and watch the Plots window.
...
# running the following code and watch your Plots window.  Here p is set to 1/2,
# after you've run the code once, try changing p to a different value.
...
binom <- function(n) {
  barplot(dbinom(0:n, size=n, prob=1/2),names.arg=0:n,ylab="Probability",
          ...
          col="light blue", ylim=c(0,0.5), xlim = c(0,25))
  Sys.sleep(0.1)
}
...
# The difference between these two commands is that sum(dbinom()) sums up the areas
# of each bar you specify, whereas pbinom sums up the bars that are less than or
...
# If you'd like to stimulate the results of 1000 people each flipping a coin 
# 50 times, type:
...
# The y-axis tells us the relative frequency of seeing different numbers of 
#	successes, in the 1000 trials. For many of the 1000 people, they get about 
#	25 tails in their 50 flips, but some do get much fewer or many more tails. 	
...
# also notice that this particular binomial distribution is 'bell shaped' like the
# normal distribution.  As a rule of thumb, whenever n*p > 10 and n*(1-p) > 10, you
# can use a normal distribution to *approximate* the binomial distribution, using
# mean = n*p and variance = n*p*(1-p).  In this case, mean = 50*0.5 = 25 and
# variance = 50*0.5*0.5 = 12.5.  The standard deviation of a normal distribution 
# is the square root of the variance. In this case, sqrt(12.5) or 3.54.  To compare 
# the approximated normal distribution to your simulated binomial data, you could type:
...
# Refs:
# Introduction to Probability with R, K Baclawski. Chapman & Hall, 2008
# Introduction to the Practice of Statistics ed 7, 
#		DS Moore, GP McCabe, & BA Craig. W.H. Freeman, 2012
...
#
#
# a) Determine the probability of each outcome (i.e., winning 0 times, 1 time,
# 	 2 times, etc)
...
barplot(dbinom(0:20, size=20, prob=18/38),names.arg=0:20,ylab="Probability",
        main="Binomial Distribution, n=20, p=18/38",xlab="Number of Wins"
       ,ylim=c(0,0.2))
...
# e)
hist(data1, freq=FALSE, breaks=seq(-0.5,21.5,1),ylim=c(0,0.2),col="grey",
		 ylab="Number of Occurences",xlab="Number of Wins in 20 plays")
...
# or
sum(data1>9)
# or just sort the numbers of wins and list, 
# and count yourself, looking at the output on your Console
sort(data1)
