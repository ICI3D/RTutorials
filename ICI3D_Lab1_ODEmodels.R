## Introduction to Infectious Disease Dynamics (Sunday session)
## Clinic on the Meaningful Modeling of Epidemiological Data
## African Institute for Mathematical Sciences, Muizenberg, RSA
##
## Juliet R.C. Pulliam, 2012
##
##
...
##  * use the package deSolve to numerically analyze a system of ODE's
...
## births and deaths). This model was discussed by Juliet and Jonathan in
## the Sunday session, and notes from these lectures are available on
## the MMED website:
##
## http://yushan.mcmaster.ca/theobio/mmed/index.php/2012_Clinic_Schedule#Sunday.2C_27_May
##
## The version of the model we'll work with here has 3 state variables (though
## only two of these need to be specified to define the state of the system, since
...
## with this tutorial (eg, see Slides 24-33  of Juliet's Sunday Lecture).

## We will now define a function that describes the change in the state variables
## with time. The function takes the input t (the current time), y (a vector
## giving the current value of the state variables - that is, their value at time
## t), and params (a vector that defines the parameter values); the function
## outputs the values of dS/dt and dI/dt at time t:
##
##-- Model 1: SIR model --##
##
...
## We will reproduce the measles outbreak model that Jim used in his "Introduction
## to spreadsheet models" example, so let our initial population size (N0) be 

N0 <- 7781984

## The second argument of the sir() function defined above is y, which I have said
## is a vector giving the current value of the state variables. As in 
## Jim's example, let's assume that the initial population has 20.5 infecteds
## and that 6.5% of the population is susceptible. (Remember that this model
## approximates individuals as continuous entities, so it is ok that these values
## are not integers!) We will define a vector pop.SI that gives the initial
## values of our 2 state variables:

pop.SI <- c(S = 0.065*N0,  # Initially 6.5% of the population is susceptible
                I = ???)       # ENTER THE NUMBER INITIALLY INFECTED (20.5)

## Notice that I've named the two values in the initial population vector to help
## us keep track of which value is which. This also allows us to take advantage
## of the with() function called within the sir() function because I have used the
## same names that are used to refer to the state variables within the function.
##
...
values["beta"]/values["gamma"] # value of R0
                               ...
                               # function as.numeric() to keep R from confusingly
                               # retaining the name of the first value.
...
## and I at some time delta.t in the future, I need to calculate the equivalent of
##
...
## It turns out that using the discrete time approximation of this process, as
## we have done on the spreadsheets, leads to rapid acummulation of error in our
## estimate of the state variables through time, even if we set our value of 
## delta.t to be very small. Luckily for us, there are a number of algorithms
## that have been developed to come up with better (both more accurate and
## more computationally efficient) approximations to the values of the state
...
## to look at the different types of arguments the function can take. There are a
## lot of them! For now, we will focus on the first 4 inputs: y, times, func,
...
## vector of parameter values). We have also defined a third argumet, func, which
## provides the model definition; it is our function sir(). All that remains is to
## define the vector of times, which tells lsoda() the timepoints for which we
## would like to know the values of S and I. (NOTE: This is different from
## specifying the time points at which to evaluate our function, which is
## determined within the lsoda() function and optimized to efficiently provide
...
## susceptibles in our equations (eg, through births). Let's see what has happened
## after a year:

subset(ts.sir,time==365)

## There are more infecteds after a year than there were at time 0, so it looks
## like it takes more than a year for the infection to burn out with these initial
## conditions. If we want to see how the epidemic has progressed through time, we
## can plot to output from lsoda(). Remember that the parameters we have used are 
## the same as in Jim's spreadsheet example, so this is a model of a measles
## epidemic in New York. We'll label the plot accordingly. (You'll learn a lot more
## about plotting - and modifying the appearance of plots - in the next tutorial.)
...
## function defining the model with births and deaths over 5 years, with the same 
## initial conditions as before. How do the dynamics compare?
##
## Question 4:
##
## Now change the value of the life expectancy for the population in the model.
## Compare the long-term dynamics of the model with different values for the life
## expectancy.
