...
require(boot); require(deSolve); require(ellipse); require(coda)
...
llikelihood <- function(parms = disease_params(), obsDat=myDat) {
...
llikelihood(trueParms) ## loglikelihood of the true parameters (which we usually never know)
llikelihood(disease_params(Beta = 3, alpha = 1))  ## vs some random guessed parameters
...
llikePrior <- function(fit.params=NULL, ## parameters to fit
    ...
    llikelihood(parms, obsDat=obsDat) + lprior(parms)
}
...
samp_Seq <- mcmcSampler(c(alpha=8, Beta=.9)
                      , seed = 1
...
## Try changing the above to have niter = 1000. How does this affect the trace plots? Use
## Gelman-Rubin diagnostic to test convergence.
## Q3
